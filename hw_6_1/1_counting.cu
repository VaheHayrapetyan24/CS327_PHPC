...
__host__ __device__ int get_char(char v) {
...
__global__ void count(char* d_in, int* d_out, int n) {
    ...
    res[get_char(cur_d_in[tid * MUL])][tid]++;
    res[get_char(cur_d_in[tid * MUL + 1])][tid]++;
    res[get_char(cur_d_in[tid * MUL + 2])][tid]++;
    res[get_char(cur_d_in[tid * MUL + 3])][tid]++;
    __syncthreads();
